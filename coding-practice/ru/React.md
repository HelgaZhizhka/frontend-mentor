...
- **Все имена — в одном стиле и легко ищутся в любой системе**
- **Нет глобальных barrel-exports и default-экспортов**
- **Git не ломается, у всех всё одинаково**
---
**Этот паттерн — максимально универсален, современен, подходит и для небольших продуктов, и для средних компаний, и для резюме/переездов на open-source.**  
Если нужно, могу свернуть раздел в ещё более резюмированный "чеклист для команды"!
---
## 2. Компоненты React
...
**❌ Плохо — `PropsWithChildren` устарел:**
```typescript
const Container = ({
  title,
  children,
...
```
**✅ Хорошо — явная типизация `children`:**
```typescript
type ContainerProps = {
  title: string
