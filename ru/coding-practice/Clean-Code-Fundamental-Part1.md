...
- **Существительные**
- **Описательные имена**
- **Избегать суффиксов Manager, Helper, Utils** (когда возможно)
**❌ Плохо:**
```typescript
...
```
**Когда Manager/Helper допустимы:**
- `CacheManager` — управляет кэшем
- `ConfigurationManager` — управляет конфигурацией
- `StringHelper` — вспомогательные функции для строк (но лучше `StringUtils` вынести в отдельные функции)
***
...
```
***
## 5. Code Smells и рефакторинг
### 5.1 Duplicated Code (Дублирование)
**Признаки:**
- Copy-paste код
...
```
***
### 5.2 Long Method (Длинная функция)
**Признаки:**
- Функция больше 20-30 строк
- Трудно понять с первого взгляда
- Несколько уровней абстракции
**Рефакторинг:** См. раздел 2.1 "Размер функции"
***
### 5.3 Large Class (Большой класс)
**Признаки:**
...
**См. раздел 2.2 "Параметры функции"**
***
### 5.5 Magic Numbers и Magic Strings
**❌ Плохо:**
```typescript
...
***
### 5.7 Deep Nesting (Глубокая вложенность)
**См. раздел 2.5 "Возврат значений"**
***
### 5.8 Primitive Obsession
